...
wrappedOperation name f (NonEmptyList (x :| xs)) =
  case L.uncons (f (x : xs)) of
    Just { head: x', tail: xs' } -> NonEmptyList (x' :| xs')
    Nothing -> unsafeCrashWith ("Impossible: empty list in NonEmptyList " <> name)
...
fromList = L.uncons >>> case _ of
  Nothing -> Nothing
  Just { head: x, tail: xs } -> Just (NonEmptyList (x :| xs))
...
snoc' l y = case L.uncons l of
  Just { head: x, tail: xs } -> NonEmptyList (x :| L.snoc xs y)
  Nothing -> singleton y
